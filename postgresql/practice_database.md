...
(4 rows)
```
10. Get all appointments with patient name, doctor name, and status, sorted by latest appointment first.
```bash
hospital_db=# select a.appointment_id, p.name, d.name,a.status,a.appointment_datetime
hospital_db-# from appointments a 
hospital_db-# join patients p
hospital_db-# on p.patient_id = a.patient_id
hospital_db-# join doctors d on d.doctor_id = a.doctor_id
hospital_db-# order by appointment_datetime desc;
 appointment_id |     name     |   name    |  status   | appointment_datetime 
----------------+--------------+-----------+-----------+----------------------
              9 | Vikas Gupta  | Dr. Khan  | Completed | 2026-04-12 12:00:00
              7 | Sneha Kapoor | Dr. Rao   | Pending   | 2026-04-10 10:30:00
              4 | Priya Verma  | Dr. Khan  | Completed | 2026-04-08 16:00:00
              8 | Vikas Gupta  | Dr. Mehta | Pending   | 2026-04-07 15:00:00
              6 | Sneha Kapoor | Dr. Gupta | Completed | 2026-04-06 13:00:00
              2 | Amit Sharma  | Dr. Khan  | Completed | 2026-04-05 14:30:00
              5 | Rahul Singh  | Dr. Mehta | Cancelled | 2026-04-03 09:30:00
              3 | Priya Verma  | Dr. Rao   | Completed | 2026-04-02 11:00:00
              1 | Amit Sharma  | Dr. Mehta | Completed | 2026-04-01 10:00:00
(9 rows)
```
11. Find total revenue generated by each doctor (only paid bills).
```bash
hospital_db=# select d.name, sum(b.total_amount) as total_revenue
hospital_db-# from appointments a
hospital_db-# join doctors d on a.doctor_id = d.doctor_id
hospital_db-# join bills b on a.appointment_id=b.appointment_id
hospital_db-# where b.payment_status = 'Paid'
hospital_db-# group by d.name;
   name    | total_revenue 
-----------+---------------
 Dr. Khan  |           800
...
(3 rows)
```
12. Get top 2 doctors with highest revenue.
```bash
hospital_db=# select d.name as doctor_name, sum(b.total_amount) as total_revenue
hospital_db-# from appointments a
hospital_db-# join doctors d on a.doctor_id=d.doctor_id
hospital_db-# join bills b on a.appointment_id=b.appointment_id
hospital_db-# where b.payment_status ilike 'paid'
hospital_db-# group by d.name
hospital_db-# order by sum(b.total_amount) desc
hospital_db-# limit 2;
...
(2 rows)
```
13. Average consultation fee per specialization
```bash
hospital_db=# SELECT 
hospital_db-# specialization,
hospital_db-# AVG(consultation_fee) AS avg_fee
hospital_db-# FROM doctors
hospital_db-# GROUP BY specialization;
  specialization   |        avg_fee        
-------------------+-----------------------
 General Physician |  500.0000000000000000
 ENT Specialist    |  900.0000000000000000
 Orthopedic        | 1200.0000000000000000
 Cardiologist      | 1050.0000000000000000
 Oncologist        | 2000.0000000000000000
 Neurologist       | 1500.0000000000000000
 Psychiatrist      | 1300.0000000000000000
 Pediatrician      |  700.0000000000000000
 Dermatologist     |  800.0000000000000000
(9 rows)
```